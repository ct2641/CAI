...
int populate_Gurobirows_generic(GRBmodel* lp, Problem_Description_C_Style* PD, Extracted_Reductions* ER)
{
	int positions[4];
	...
	for (int cur_pair = 0; cur_pair < ER->num_pairs; cur_pair++)
	{
		...
		for (int base = 0; base < comp_length; base++)
		{
			...
			int num_validpos = ER->Map_Compress(positions, doublevalues, 4);
	...
	for (int i = 0; i < PD->num_rvs; i++) {
		...
		doublevalues[0] = 1.0;
		doublevalues[1] = -1.0;
		int num_validpos = ER->Map_Compress(positions, doublevalues, 2);
		if (num_validpos != 0) { // adding new constraints
			if ((status = status | GRBaddconstr(lp, num_validpos, positions, doublevalues, GRB_GREATER_EQUAL, rhs, NULL)))
				printf("Something wrong in add rows\n");
			count++;
...
int add_Gurobirows_problemspecific(GRBmodel* lp, Problem_Description_C_Style* PD, Extracted_Reductions* ER)
{
	...
	// to add objective
	if (PD->num_items_in_objective > 0 && PD->num_items_in_objective <= MAXINDPITEMS_C_STYLE) {
		...
		int num_effective = ER->Map_Compress(positions, doublevalues, PD->num_items_in_objective);
		if (num_effective > 0) {
			//**********************************************************************/
			//*********************specific for Cplex*******************************/
			//**********************************************************************/
	...
	if (PD->num_independence > 0) {
		for (int i = 0; i < PD->num_independence; i++) {
			...
			int num_effective = ER->Map_Compress(positions, doublevalues, PD->list_num_items_in_independence[i]);
			if (num_effective > 0) {
				//**********************************************************************/
				//*********************specific for Cplex*******************************/
				//**********************************************************************/
	...
	if (PD->num_constantbounds > 0) {
		for (int i = 0; i < PD->num_constantbounds; i++) {
			if (PD->list_type_constantbounds[i] == 0 || PD->list_type_constantbounds[i] == 2) {
				...
				int num_effective = ER->Map_Compress(positions, doublevalues, PD->list_num_items_in_constantbounds[i]);
				if (num_effective > 0) {
					//**********************************************************************/
					//*********************specific for Cplex*******************************/
					//**********************************************************************/
	...
	GRBupdatemodel(lp);
	int num_rows;
	GRBgetintattr(lp, "NumConstrs", &num_rows);
	printf("Total number of constraints given to Gurobi: %d\n", num_rows);
	return(0);
}

int populate_Gurobirows_random(GRBmodel* lp, Problem_Description_C_Style* PD, Extracted_Reductions* ER, int num_constraints)
{
	int positions[4];
	double initial_values[4] = { 1,1,-1,-1 };
	double doublevalues[4];
...
void print_Gurobiinequalities(GRBmodel* primallp, GRBmodel* duallp, Problem_Description_C_Style* PD, Extracted_Reductions* ER)
{
