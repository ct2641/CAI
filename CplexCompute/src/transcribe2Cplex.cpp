...
int add_Cplexrows_problemspecific(CPXENVptr env, CPXLPptr lp, CAI::Problem_Description_C_Style* PD, Extracted_Reductions* ER)
{
	...
	if (PD->num_independence > 0) {
		for (int i = 0; i < PD->num_independence; i++) {
			...
			if (num_effective > 0) {
				...
				// also add the negative term, this is useful in generating the proof but a little wasteful in terms of the LP itself
	...
	printf("Total number of constraints given to Cplex: %d\n", CPXgetnumrows(env,lp));
