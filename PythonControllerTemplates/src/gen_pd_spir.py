...
"""
A variation of the the problem in Tian, Chao. "On the storage cost of private information retrieval." IEEE Transactions on Information Theory (2020).
for the symmetric PIR setting. 
"""

parser = argparse.ArgumentParser(description = 'PIR with N servers and K messages')
parser.add_argument('-N', '--N', type = int, default = 2, choices = range(2, 10), help = "number of servers")
...
def gen_pd_spir(N, K):
    ...
    fn = cai + 'PlotTradeoff/inputfiles/PD_SPIR{}x{}.pd'.format(N, K)
    if os.path.exists(fn):
        ...
        if input() == "y":
            ...
            print("Keep old PD_SPIR{}x{}.pd".format(N, K))
            return fn
    ...
    PD["BC"].append("H(W1) = 1")
    for k in range(1, K + 1):
        PD["BC"].append("I(" + ",".join(W[:k-1] + W[k:]) + ";" + "".join(["Q{}{},".format(n, k) for n in range(1, N + 1)])
            + "".join(["A{}{},".format(n, k) for n in range(1, N + 1)]) + "F) = 0")
    ...
    for Nperm in Nperms:
        for Kperm in Kperms:
            ...
            PD["S"].append(["F", "S"] + Wperm + Sperm + Qperm + Aperm)
